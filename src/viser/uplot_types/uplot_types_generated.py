...
Select = TypedDict('Select', {
    # div into which .u-select will be placed: .u-over or .u-under.
    'over': bool
}, total=False)

Cursor_Bind = TypedDict('Cursor_Bind', {
...
Cursor_Drag = TypedDict('Cursor_Drag', {
    'setScale': bool,
...
Cursor_Focus = TypedDict('Cursor_Focus', {
...
Series_Points = TypedDict('Series_Points', {
...
Series_Facet = TypedDict('Series_Facet', {
    'scale': Required[str],
    'auto': bool,
    'sorted': Sorted
}, total=False)
...
Axis_Ticks = TypedDict('Axis_Ticks', {
    # length of tick in CSS pixels.
    'size': float
}, total=False)

Plugin = TypedDict('Plugin', {
    'hooks': Required[ComplexType],
    # can mutate provided opts as necessary.
    'opts': ComplexType
}, total=False)

Cursor_Sync = TypedDict('Cursor_Sync', {
    ...
    'match': tuple[JSCallback, JSCallback, JSCallback],
    # event filters.
    'filters': UnknownType,
    # sync scales' values at the cursor position (exposed for read-back by subscribers).
    'values': list[str | float | None] | JSCallback | str | list[list[str | float | None]]
}, total=False)

Legend = TypedDict('Legend', {
    'show': bool,
    ...
    'values': list[str | JSCallback]
}, total=False)

Cursor_Points = TypedDict('Cursor_Points', {
    'show': bool | JSCallback,
...
Series = TypedDict('Series', {
    ...
    'gaps': list[tuple[float, float]] | JSCallback,
...
Options = TypedDict('Options', {
    ...
    'data': list[UnknownType] | ComplexType,
    ...
    'padding': tuple[float | None | JSCallback, float | None | JSCallback, float | None | JSCallback, float | None | JSCallback],
    'select': Select,
    'legend': Legend,
    'cursor': Cursor,
    'focus': Focus,

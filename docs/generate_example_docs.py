...
async def run_example_and_capture(
    ...
    try:
        ...
        success = await capture_screenshot_playwright(
            f"http://localhost:{port}/?dummyWindowDimensions=fill&hideViserLogo=&dummyWindowTitle=viser&initialCameraPosition=1.5,1.5,1.5",
            output_path,
        )
...
def generate_code_pages(
    ...
    for example_name, example_path, title, desc, full_docstring in examples:
        ...
        with open(rst_path, "w") as f:
            ...
            # Use full docstring for better explanation, fallback to desc
            explanation = full_docstring if full_docstring else desc
            if explanation:
                # Format the docstring nicely for RST
                explanation_lines = explanation.split("\n")
                if len(explanation_lines) > 1:
                    # Multi-line docstring - first line is title, rest is description
    ...
    with open(index_path, "w") as f:
        ...
        for example_name, _, _, _, _ in sorted(examples):
            safe_name = example_name.replace("/", "_")
            f.write(f"   {safe_name}\n")
    ...
    with open(include_file, "w") as f:
        ...
        for category in sorted_categories:
            ...
            for example_name, example_path, title, desc, full_doc in grouped[category]:
                ...
                category_mapping = {
                    "00_getting_started": "examples_getting_started",
                    "01_scene": "examples_scene",
                    "02_gui": "examples_gui",
                    "03_interaction": "examples_interaction",
                    "04_demos": "examples_demos",
                }
                category_dir = category_mapping.get(
                    category, "examples_getting_started"
                )
...
def generate_organized_rst_files(examples: List[Tuple[str, Path, str, str, str]]):
    ...
    for example_name, example_path, title, desc, full_docstring in examples:
        ...
        with open(rst_path, "w") as f:
            ...
            # Add screenshot - adjust path based on directory depth for dirhtml builds
            screenshot_path = f"../../../_static/examples/{safe_name}.png"
            f.write(".. figure:: " + screenshot_path + "\n")
            f.write("   :width: 100%\n")
            f.write("   :alt: " + title + "\n\n")
...
def generate_subdirectory_indexes(examples: List[Tuple[str, Path, str, str, str]]):
    ...
    for example_name, example_path, title, desc, full_docstring in examples:
        ...
        category_dir = category_mapping.get(category_key, "examples_getting_started")
        safe_name = example_name.replace("/", "_")

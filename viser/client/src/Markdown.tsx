...
import { Anchor, Blockquote, Code, Image, List, ListProps, Table, Text, Title, TitleOrder } from '@mantine/core'
...
function rehypeCodeblock(): void | Transformer<Root, Root> {
    return (tree) => {
        visit(tree, 'element', (node: Element, i: number | null, parent: Root | Element | null) => {
...
function MdxTitle(props: React.ComponentPropsWithoutRef<typeof Title>, order: TitleOrder) {
    return <Title order={order} {...props}></Title>;
}

function MdxList(props: Omit<React.ComponentPropsWithoutRef<typeof List>, 'children' | 'type'>, children: React.ComponentPropsWithoutRef<typeof List>['children'], type: ListProps["type"]) {
    // Account for GFM Checkboxes
    if (props.className == "contains-task-list") {
        return <List type={type} {...props} listStyleType='none'>{children}</List>;
    }
    return <List type={type} {...props}>{children}</List>;
}

function MdxListItem(props: Omit<React.ComponentPropsWithoutRef<typeof List.Item>, 'children'>, children: React.ComponentPropsWithoutRef<typeof List.Item>['children']) {
    return <List.Item {...props}>{children}</List.Item>;
}

// A possible improvement is to use Mantine Prism to add code highlighting support.
function MdxCode(props: Omit<React.ComponentPropsWithoutRef<typeof Code>, 'children'>, children: React.ComponentPropsWithoutRef<typeof Code>['children']) {
    return <Code {...props}>{children}</Code>;
}

function MdxBlockquote(props: React.ComponentPropsWithoutRef<typeof Blockquote>) {
    return <Blockquote {...props} />;
}

function MdxCite(props: React.DetailedHTMLProps<React.HTMLAttributes<HTMLElement>, HTMLElement>) {
    return <cite style={{ display: "block", fontSize: "0.875rem", marginTop: "0.625rem", color: "#909296", overflow: "hidden", textOverflow: "ellipsis" }} {...props} />
}
...
async function parseMarkdown(markdown: string) {
    ...
    const { default: Content } = await evaluate(markdown, { ...runtime, ...provider, development: false, remarkPlugins: [remarkGfm], rehypePlugins: [rehypeCodeblock, rehypeColorChips] })
    return Content;
}

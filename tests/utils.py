...
def sample_transform(Group: Type[T], batch_axes: Tuple[int, ...] = ()) -> T:
    ...
    if strategy == 0:
        ...
        return cast(
            T,
            Group.sample_uniform(onp.random.default_rng(seed), batch_axes=batch_axes),
        )
    elif strategy == 1:
...
def general_group_test(
    f: Callable[[Type[vtf.MatrixLieGroup], Tuple[int, ...]], None],
    max_examples: int = 30,
) -> Callable[[Type[vtf.MatrixLieGroup], Tuple[int, ...], Any], None]:
    """Decorator for defining tests that run on all group types."""

    # Disregard unused argument.
    def f_wrapped(
        Group: Type[vtf.MatrixLieGroup], batch_axes: Tuple[int, ...], _random_module
    ) -> None:
        f(Group, batch_axes)
